...
class artery:
    def __init__(self, elasticity_func, relaxed_radius_func, delta_b, Ru=1, Rd=2, L=6, Reynolds_no=2, E=3, h=6, q_0=3,
        ...
        self.pinn = PINN(self.R_network, self.q_network).build(self.delta_b, E, h, self.elasticity_func, 5, 6, 7, Ru, Rd, L, Reynolds_no, q_0)
        self.pinn.summary()
    ...
    def create_dataset(self):
        ...
        q_bndry_inflow = initial_q(t_outflow, self.timeperiod, self.tow)
...
class p_class(tf.keras.layers.Layer):
  ...
  """
  def __init__(self,R_model,q_model,Ru,Rd,L,**kwargs):
    """
      Args:
  ...
  def call(self,input):
    ...
    with tf.GradientTape() as g:
      ...
      p = (4/3)*((E*h)/relaxed_radius_func(z_outflow,self.Ru,self.Rd,self.L)) * (1 - tf.sqrt(A_bo_0/A_bo))
      #p=1
...
class PINN:
  ...
  def build(self,delta_b,E,h,elasticity_func,R1,R2,CT,Ru,Rd,L,Reynolds_no,q_0):
    ...
    #print(df_dr0)
    #print("T2")
    t2 = Lambda(lambda ar: math.sqrt(math.pi) * elasticity_func(relaxed_radius_func(ar[0],int(ar[3]),int(ar[4]),int(ar[5]))) + tf.math.sqrt(ar[1]) * ar[2])((z,A_0,df_dr0,Ru,Rd,L))
    ...
    p_obj = p_class(self.R_network,self.q_network,Ru,Rd,L)
